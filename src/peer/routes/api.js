...
export function createPeerApi(runtime, config) {
  ...
  router.post('/messages/direct', async (req, res, next) => {
    try {
      ...
      if (!toPeerId || !content) return res.status(400).json({ error: 'toPeerId and content are required' });
  ...
  router.post('/messages/group', async (req, res, next) => {
    try {
      ...
      if (!groupId || !content) return res.status(400).json({ error: 'groupId and content are required' });
  ...
  router.post('/groups/:groupId/members', async (req, res, next) => {
    try {
      const group = await runtime.addGroupMembers(req.params.groupId, req.body.members ?? []);
      res.json({ group });
    } catch (error) {
      next(error);
    }
  ...
  router.post('/files', upload.single('file'), async (req, res, next) => {
    try {
      ...
      if (!toPeerId || !req.file) return res.status(400).json({ error: 'toPeerId and file are required' });
